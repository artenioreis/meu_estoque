document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!apiUrlContainer) {
        console.error("DIAGNÓSTICO: O elemento 'api-url-container' não foi encontrado no HTML.");
        return;
    }
    ...
    if (!apiUrl || apiUrl.includes('{'.concat('{'))) {
        console.error("DIAGNÓSTICO: A URL da API não foi renderizada. Verifique se o servidor Flask está a ser executado.");
        return;
    }

    console.log("DIAGNÓSTICO: A iniciar a busca de dados em:", apiUrl);
    ...
    const showNoDataMessage = (chartId, messageId) => {
        ...
        console.warn(`DIAGNÓSTICO: Não há dados para o gráfico '${chartId}'. A exibir mensagem.`);
    };
    ...
    fetch(apiUrl)
        .then(response => {
            ...
            console.log("DIAGNÓSTICO: Resposta da rede recebida com sucesso.");
            return response.json();
        })
        .then(data => {
            console.log("DIAGNÓSTICO: Dados recebidos da API:", JSON.stringify(data, null, 2));
            ...
            if (data.vendas_diarias && data.vendas_diarias.data.some(v => v > 0)) {
                ...
                if (ctxVendas) new Chart(ctxVendas, { type: 'line', data: { labels: data.vendas_diarias.labels, datasets: [{ label: 'Nº de Vendas', data: data.vendas_diarias.data, fill: true, backgroundColor: 'rgba(153, 102, 255, 0.2)', borderColor: 'rgba(153, 102, 255, 1)', tension: 0.1 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
            } else {
            ...
            if (data.contas_pagar && data.contas_pagar.labels.length > 0) {
                ...
                if (ctxContas) new Chart(ctxContas, { type: 'doughnut', data: { labels: data.contas_pagar.labels, datasets: [{ label: 'Status', data: data.contas_pagar.data, backgroundColor: ['rgba(255, 159, 64, 0.7)', 'rgba(75, 192, 192, 0.7)', 'rgba(255, 99, 132, 0.7)'], borderColor: '#fff', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } } } });
            } else {
            ...
            if (data.estoque && data.estoque.labels.length > 0) {
                ...
                if (ctxEstoque) new Chart(ctxEstoque, { type: 'bar', data: { labels: data.estoque.labels, datasets: [{ label: 'Quantidade em Estoque', data: data.estoque.data, backgroundColor: 'rgba(54, 162, 235, 0.6)', borderColor: 'rgba(54, 162, 235, 1)', borderWidth: 1 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
            } else {
            ...
            if (data.mais_vendidos && data.mais_vendidos.labels.length > 0) {
                ...
                if (ctxMaisVendidos) new Chart(ctxMaisVendidos, { type: 'bar', data: { labels: data.mais_vendidos.labels, datasets: [{ label: 'Unidades Vendidas', data: data.mais_vendidos.data, backgroundColor: 'rgba(75, 192, 192, 0.6)', borderColor: 'rgba(75, 192, 192, 1)', borderWidth: 1 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
            } else {
                showNoDataMessage('graficoMaisVendidos', 'no-data-vendidos');
            }
        })
        .catch(error => {
            console.error('DIAGNÓSTICO: Houve um problema com a operação fetch:', error);
        });
});
